...
Whether tuple structs exist, and what `Parts` is. `struct
Parts(List[PartsItem])` in `assembly.kitty` is the only tuple struct in
the examples; positional construction of any struct already exists
...
The options: keep tuple structs; drop them and make `Parts` a struct
