...
use kitty_cst::CstNode;
use kitty_lexer::{lex, Token};
use kitty_syntax::{NodeKind, SyntaxBuilder, SyntaxTree, TokenKind};
use std::fmt;
...
fn process_events(input: &str, events: &[Event], tokens: &[Token]) -> SyntaxTree {
    Sink {
...
impl Sink<'_> {
    ...
    fn skip_trivia(&mut self) {
        if self.cursor < self.tokens.len() && self.tokens[self.cursor].kind == TokenKind::Whitespace
        {
            self.add_token();
        }
    }
